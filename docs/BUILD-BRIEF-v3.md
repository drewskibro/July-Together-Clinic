...
- [x] **Stripe connected** — account "AT Health Ltd / Together Clinic" (`acct_1Trkqj…`), payments + payouts enabled, descriptor TOGETHER CLINIC, currently in **test mode** (correct for the build).
- [ ] **Enable Stripe manual capture** — WooCommerce → Stripe → "Issue an authorization on checkout, and capture later." (Option B reinstates this; v2's blanket "no change needed" no longer applies.) Enable in test mode now; it carries to live.
- [ ] **Switch test → live** only after the joint test order passes (owner + Drew together).
- [ ] Confirm the **Click & Drop 7-day import window** keys off order-created vs paid date (on the test order) — informs the late-fallback-payment handling.
...
| **Hold expiry timing (owner sign-off, 2026-09-11)** | **Per-hold, not fixed.** Read the authorisation expiry Stripe reports for *each* PaymentIntent and schedule the reminder (≈48h before) and the release *relative to that timestamp* — never a hardcoded 7 days, since some cards get a shorter window. |
| **Late fallback payment vs Click & Drop 7-day import window (owner sign-off, 2026-09-11)** | Click & Drop only imports orders ≤7 days old and only in `Processing`. A fallback-link payment captured after the hold expired can land on an order older than that window → must not get stuck. Handling: on late capture, (a) fire a "manual dispatch check" notice to the pharmacy so it can never fail silently, and (b) stamp the dispatch-relevant order date to the capture date so Click & Drop re-sees it as current. **Open:** confirm whether Click & Drop keys its 7-day window off order-created or paid date (decides whether the date-stamp alone suffices) — to confirm on the go-live test order. |
| **Identity verification (owner sign-off, 2026-09-11)** | Stripe Identity, **document + selfie/liveness** (selfie included in the per-check price), as the final step after payment; **once per patient** (first order, not reorders); enforce **18+**; **cross-check the name + DOB Stripe returns against the assessment-entered values** and flag any mismatch to the prescriber exactly like a failed check; failures/mismatches do **not** auto-reject — order goes to the prescriber flagged, hold released if declined. Selfie = biometric special-category data → **privacy notice must cover it** (Stripe as processor, lawful basis/explicit consent) — drafted for the owner's sign-off. |
| **Foundayo (orforglipron) tablets (owner sign-off, 2026-09-11)** | New treatment, tablets, monthly (×30). Ladder + prices: 0.8mg £99 · 2.5mg £119 · 5.5mg £129 · 9mg £159 · 14.5mg £189 · 17.2mg £199. Starter 0.8mg; ±1 reorder gate applies. **Switching:** content-only, not pushed — new Foundayo patients start at 0.8mg, **no** dose-conversion matrix to/from Foundayo. Page + eligibility flow + safety content drafted in the Wegovy-Tablets style for the owner's **clinical sign-off**; built switched-off, flipped live on sign-off. Licensed and available now (owner confirmed). |
